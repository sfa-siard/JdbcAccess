...
Enhanced by Hartwig Thomas adding dropTable and dropQuery.
*/
...
public class DatabaseImpl implements Database {
    ...
    static final boolean BROKEN_NIO = Boolean.TRUE.toString()
                                                  .equalsIgnoreCase(
    ...
    private static final Collection<String> SYSTEM_CATALOG_COLUMNS =
    ...
    private static final Collection<String> SYSTEM_CATALOG_TABLE_DETAIL_COLUMNS =
            new HashSet<String>(Arrays.asList(CAT_COL_NAME, CAT_COL_TYPE, CAT_COL_ID,
                                              CAT_COL_FLAGS, CAT_COL_PARENT_ID,
                                              CAT_COL_DATABASE, CAT_COL_FOREIGN_NAME));
    /** the columns to read when getting object propertyes */
    private static final Collection<String> SYSTEM_CATALOG_PROPS_COLUMNS =
    ...
    public static void validateIdentifierName(String name,
        ...
        if (INVALID_IDENTIFIER_CHARS.matcher(name)
                                    .find()) {
    ...
    private boolean matchesLinkedTable(Table table, String linkedTableName,
        ...
        return (table.getName()
                     .equalsIgnoreCase(linkedTableName) &&
                (_linkedDbs != null) &&
  ...
  ====================================================================*/
    ...
    /**
     * return name of any row in MSysObjects for which the given object is the parent
     * @param sParentName name of the parent object.
     * @return name of the first child object found.
     * @throws IOException
     */
    private String getChildName(String sParentName)
            throws IOException {
        String sChildName = null;
        Integer iObjectId = null;
        if (_systemCatalog == null)
            _systemCatalog = getSystemTable(TABLE_SYSTEM_CATALOG);
        /* search system catalog for name and get its id */
        Cursor cursor = new CursorBuilder(_systemCatalog).toCursor();
        for (Row row = cursor.getNextRow(); (iObjectId == null) && (row != null); row = cursor.getNextRow()) {
            if (sParentName.equals(row.getString(CAT_COL_NAME)))
                iObjectId = row.getInt(CAT_COL_ID);
        }
        if (iObjectId != null) {
            /* find an object, for which it is the parent id */
            cursor.reset();
            for (Row row = cursor.getNextRow(); (sChildName == null) && (row != null); row = cursor.getNextRow()) {
                Integer iParentId = row.getInt(CAT_COL_PARENT_ID);
                if (iParentId.intValue() == iObjectId.intValue())
                    sChildName = row.getString(CAT_COL_NAME);
            }
        }
        return sChildName;
    }
    ...
    /**
     * delete the ACEs (access control entries) associated with page number.
     * @param iPageNumber page number (object id) of table.
     * @throws IOException if an I/O error occurs.
     */
    private void deleteAccessControlEntries(int iPageNumber)
            throws IOException, SQLException {
        TableImpl tiAcEntries = getAccessControlEntries();
        Cursor cursor = new CursorBuilder(tiAcEntries).toCursor();
        for (Row row = cursor.getNextRow(); row != null; row = cursor.getNextRow()) {
            Integer iObjectId = row.getInt(ACE_COL_OBJECT_ID);
            if (iObjectId.intValue() == iPageNumber) {
                cursor.deleteCurrentRow();
                if (!cursor.isCurrentRowDeleted())
                    throw new SQLException("Could not delete ACE!", "HY000");
            }
        }
    }

    /**
     * checks, whether the table is referenced in a relationship.
     * @param table table
     * @return true, if it is referenced.
     * @throws IOException
     */
    private boolean isReferenced(Table table)
            throws IOException {
        boolean bReferenced = false;
        for (Relationship rel : getRelationships()) {
            if ((table.getName()
                      .equals(rel.getFromTable()
                                 .getName())) || (table.getName()
                                                       .equals(rel.getToTable()
                                                                  .getName())))
                bReferenced = true;
        }
        return bReferenced;
    ...
    public void dropRelationship(String sDropName)
            throws IOException, SQLException {
        /* delete all rows with this name from MSysRelationships */
        if (_relationships == null)
            _relationships = getSystemTable(TABLE_SYSTEM_RELATIONSHIPS);
        Cursor cursor = new CursorBuilder(_relationships).toCursor();
        for (Row row = cursor.getNextRow(); row != null; row = cursor.getNextRow()) {
            if (sDropName.equals(row.getString(REL_COL_NAME))) {
                /** for debugging ...
                 String sFromName = row.getString(REL_COL_FROM_TABLE);
                 String sToName = row.getString(REL_COL_TO_TABLE);
                 **/
                cursor.deleteCurrentRow();
                if (!cursor.isCurrentRowDeleted())
        ...
        for (Row row = cursor.getNextRow(); row != null; row = cursor.getNextRow()) {
            if (sDropName.equals(row.getString(CAT_COL_NAME))) {
                Integer iId = row.getInt(CAT_COL_ID);
                cursor.deleteCurrentRow();
                deleteAccessControlEntries(iId.intValue());
            }
        }
    }
     ...
     * @throws IOException if an I/O error occurs.
     * @throws SQLException if relationships could not be deleted.
     */
    private void dropRelationships(String sTableName)
            throws IOException, SQLException {
        Set<String> setRelationshipNames = new HashSet<String>();
        if (_relationships == null)
            _relationships = getSystemTable(TABLE_SYSTEM_RELATIONSHIPS);
        Cursor cursor = new CursorBuilder(_relationships).toCursor();
        for (Row row = cursor.getNextRow(); row != null; row = cursor.getNextRow()) {
            String sFromName = row.getString(REL_COL_FROM_TABLE);
            String sToName = row.getString(REL_COL_TO_TABLE);
            if (sFromName.equals(sTableName) || sToName.equals(sTableName))
                setRelationshipNames.add(row.getString(REL_COL_NAME));
        }
        for (Iterator<String> iterRelationshipName = setRelationshipNames.iterator(); iterRelationshipName.hasNext(); ) {
            String sRelationshipName = iterRelationshipName.next();
            dropRelationship(sRelationshipName);
        }
    }

    /**
     * drops an (empty) table from the system catalog
     * and removes all relationships involving it
     * if no SELECT Queries refer to it.
     * @param sDropName name of table to be dropped.
     * @throws IOException for I/O errors.
     * @throws SQLException if table could not be dropped.
     */
    public void dropTable(String sDropName)
            throws IOException, SQLException {
        /* make sure, table info is loaded */
        Table table = getTable(sDropName);
        if (table != null) {
            if (table.getRowCount() == 0) {
                if ((!isReferenced(table)) && (getChildName(sDropName) == null)) {
                    /* use internal table lookup to find the page number */
                    String sDropTableLookup = toLookupName(sDropName);
                    TableInfo ti = _tableLookup.get(sDropTableLookup);
                    Integer iPageNumber = ti.pageNumber;
                    /* remove it from lookup if it was there */
                    _tableLookup.remove(sDropTableLookup);
                    /* remove it from the table cache if it was there */
                    _tableCache._tables.remove(iPageNumber);
                    /* remove it from the system catalog */
                    TableImpl tableSysObjects = getSystemCatalog();
                    Cursor cursor = CursorBuilder.createCursor(tableSysObjects);
                    boolean bFound = cursor.findFirstRow(Collections.singletonMap(CAT_COL_NAME, sDropName));
                    if (bFound) {
                        Row row = cursor.getCurrentRow();
                        tableSysObjects.deleteRow(row);
                        if (!cursor.isCurrentRowDeleted())
                            throw new SQLException("Row with value \"" + sDropName + "\" in column \"" + CAT_COL_NAME + "\" could not be deleted!", "HY000");
                        deleteAccessControlEntries(iPageNumber);
                        dropRelationships(sDropName);
                    } else
                        throw new SQLException("Row with value \"" + sDropName + "\" in column \"" + CAT_COL_NAME + "\" not found in table \"" + tableSysObjects.getName() + "!", "HY000");
                } else
                    throw new SQLException("Table " + sDropName + " cannot be dropped, because it is referenced!", "90107");
            } else
                throw new SQLException("Table " + sDropName + " cannot be dropped, because it is not empty!", "90118");
        } else
            throw new SQLException("Table " + sDropName + " not found!", "42S02");
    }

    /**
     * delete the query with the given object id.
     * @param iObjectId object id of query.
     * @throws IOException, if an I/O error occurs.
     */
    private void dropQuery(int iObjectId)
            throws IOException, SQLException {
        /* delete all rows with this object id from MSysQueries */
        if (_queries == null)
            _queries = getSystemTable(TABLE_SYSTEM_QUERIES);
        Cursor cursor = new CursorBuilder(_queries).toCursor();
        for (Row row = cursor.getNextRow(); row != null; row = cursor.getNextRow()) {
            Integer iId = row.getInt(QueryFormat.COL_OBJECTID);
        ...
        for (Row row = cursor.getNextRow(); row != null; row = cursor.getNextRow()) {
            Integer iId = row.getInt(CAT_COL_ID);
            if (iId.intValue() == iObjectId) {
                deleteAccessControlEntries(iObjectId);
                cursor.deleteCurrentRow();
            }
        }
    }
     ...
     * @param sDropName name of query to be deleted.
     * @throws IOException, if an error occurs.
     * @throws SQLException, if the query cannot be deleted.
     */
    public void dropQuery(String sDropName)
            throws IOException, SQLException {
        if (getChildName(sDropName) == null) {
            List<Query> listQueries = getQueries();
            for (Iterator<Query> iterQuery = listQueries.iterator(); iterQuery.hasNext(); ) {
                Query query = iterQuery.next();
                if (query.getName()
                         .equals(sDropName)) {
                    int iObjectId = query.getObjectId();
                    /* remove all rows in system tables with this object id */
                    dropQuery(iObjectId);
                }
            }
        } else
    ...
    public List<Relationship> getRelationships(
            ...
            throws IOException {
        int nameCmp = table1.getName()
                            .compareTo(table2.getName());
        if (nameCmp == 0) {
            throw new IllegalArgumentException(withErrorContext(
    ...
    RelationshipImpl writeRelationship(RelationshipCreator creator)
        ...
        int numCols = newRel.getFromColumns()
                            .size();
        ...
        for (int i = 0; i < numCols; ++i) {
            ...
            fromTableCol.setRowValue(row, newRel.getFromTable()
                                                .getName());
            fromColCol.setRowValue(row, newRel.getFromColumns()
                                              .get(i)
                                              .getName());
            toTableCol.setRowValue(row, newRel.getToTable()
                                              .getName());
            toColCol.setRowValue(row, newRel.getToColumns()
                                            .get(i)
                                            .getName());
            rows.add(row);
        }
    ...
    private String createRelationshipName(RelationshipCreator creator)
        ...
        String origName = creator.getPrimaryTable()
                                 .getName();
        if (origName.length() > limit) {
            origName = origName.substring(0, limit);
        }
        limit = maxIdLen - 3;
        origName += creator.getSecondaryTable()
                           .getName();
        if (origName.length() > limit) {
            origName = origName.substring(0, limit);
        }
        ...
        for (Row row :
                CursorImpl.createCursor(_systemCatalog)
                          .newIterable()
                          .setColumnNames(
        ...
        if (creator.hasReferentialIntegrity()) {
            ...
            for (Index idx : creator.getSecondaryTable()
                                    .getIndexes()) {
    ...
    private void addToSystemCatalog(String name, int objectId, Short type,
        ...
        for (Iterator<ColumnImpl> iter = _systemCatalog.getColumns()
                                                       .iterator();
             iter.hasNext(); idx++) {
    ...
    private abstract class TableFinder {
        ...
        public void getTableNames(Set<String> tableNames,
                                  ...
                                  boolean linkedTables)
                throws IOException {
            for (Row row : getTableNamesCursor().newIterable()
                                                .setColumnNames(
                                                        SYSTEM_CATALOG_COLUMNS)) {
    ...
    private final class DefaultTableFinder extends TableFinder {
        ...
        protected Cursor getTableNamesCursor() throws IOException {
            return _systemCatalogCursor.getIndex()
                                       .newCursor()
    ...
    private final class FallbackTableFinder extends TableFinder {
        ...
        public TableInfo lookupTable(String tableName) throws IOException {
            ...
            for (Row row : _systemCatalogCursor.newIterable()
                                               .setColumnNames(
                                                       SYSTEM_CATALOG_TABLE_DETAIL_COLUMNS)) {
