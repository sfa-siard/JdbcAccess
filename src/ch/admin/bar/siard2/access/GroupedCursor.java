...
public class GroupedCursor implements ResultSetCursor
{
    ...
    throws IOException
  {
    ...
    _rsc.beforeFirst();
    List<GroupingElement> listGrouping = _ss.getQuerySpecification().getGroupingElements();
    int iRow = 0;
    if (_rsc.getCount() > 0)
    {
      for (Row row = _rsc.getNextRow(); row != null; row = _rsc.getNextRow())
      {
        ...
        {
          BooleanValueExpression bve = _ss.getQuerySpecification().getHavingCondition();
          if (bve != null)
        ...
        if (bInclude)
        {
          ...
          if ((listGrouping != null) && (listGrouping.size() > 0))
          {
            for (int iGroupingElement = 0; iGroupingElement < listGrouping.size(); iGroupingElement++)
            {
              ...
              IdChain idcColumn = ge.getOrdinaryGroupingSets().get(0);
              String sColumnName = idcColumn.get().get(idcColumn.get().size()-1); 
  ...
  {
    List<SelectSublist> listSelects = _ss.getQuerySpecification().getSelectSublists();
    /* resetAggregates of all select sub lists */
    for (int i = 0; i < listSelects.size(); i++)
    {
      SelectSublist ss = _ss.getQuerySpecification().getSelectSublists().get(i);
    ...
    {
      if (iRow == listGroup.get(iIndex).intValue())
      {
        iIndex++;
